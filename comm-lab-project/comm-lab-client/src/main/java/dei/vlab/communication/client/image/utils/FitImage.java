...
public class FitImage extends Image {
	...
	private void resize() {
		...
		} else if ((maxWidth != null) && (maxWidth < actualWidth || stretch)) {
			if (maxHeight != null) {
				if (aspectRatio != null) {
		...
		} else if ((maxHeight != null) && (maxHeight < actualHeight || stretch)) {
			setHeight(maxHeight);
			if (aspectRatio != null)
				setWidth((int) Math.round(maxHeight / aspectRatio));
			else
	...
	public FitImage(String url, int maxWidth, int maxHeight, Boolean stretch) {
		super();
		this.maxWidth = maxWidth;
		this.maxHeight = maxHeight;
		...
		ImagePreloader.load(url, new ImageLoadHandler() {
			public void imageLoaded(ImageLoadEvent event) {
				if (!event.isLoadFailed()) {
					...
					actualHeight = dimensions.getHeight();
					actualWidth = dimensions.getWidth();
