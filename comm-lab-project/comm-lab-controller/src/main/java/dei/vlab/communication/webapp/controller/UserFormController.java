...
public class UserFormController extends BaseFormController {
    ...
    public String onSubmit(User user, BindingResult errors, HttpServletRequest request,
        ...
        if (request.getParameter("delete") != null) {
            ...
            saveMessage(request, getText("user.deleted", user.getUserDetail().getFullName(), locale));
            ...
            if (request.isUserInRole(Role.USER_TYPE_ADMIN)) {
            ...
            try {
                ...
                errors.rejectValue("username", "errors.existing.user",
                        new Object[]{user.getUsername(), user.getUserDetail().getEmail()}, "duplicate user");
            ...
            if (!StringUtils.equals(request.getParameter("from"), "list")) {
                saveMessage(request, getText("user.saved", user.getUserDetail().getFullName(), locale));

                // return to main Menu
                return getCancelView();
            } else {
                if (StringUtils.isBlank(request.getParameter("version"))) {
                    saveMessage(request, getText("user.added", user.getUserDetail().getFullName(), locale));

                    // Send an account information e-mail
                    message.setSubject(getText("signup.email.subject", locale));

                    try {
                        sendUserMessage(user, getText("newuser.email.message", user.getUserDetail().getFullName(), locale),
                    ...
                    saveMessage(request, getText("user.updated.byAdmin", user.getUserDetail().getFullName(), locale));
                }
            }
        }
    ...
    protected User showForm(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!request.isUserInRole(Role.USER_TYPE_ADMIN) && !isFormSubmission(request)) {
        ...
        if (!isFormSubmission(request)) {
            ...
            if (userId == null && !isAdd(request)) {
                ...
                user = new User();
                user.addRole(new Role(Role.ROLE_BEGINNER));
            }
